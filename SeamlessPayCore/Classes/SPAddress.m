...
- (instancetype)initWithPaymentMethodBillingDetails:(SPPaymentMethodBillingDetails *)billingDetails {
    self = [super init];
    if (self) {
...
- (BOOL)containsRequiredShippingAddressFields:(NSSet<SPContactField> *)requiredFields {
    ...
    if ([requiredFields containsObject:SPContactFieldEmailAddress]) {
        containsFields = containsFields && [SPEmailAddressValidator stringIsValidEmailAddress:self.email];
    }
    if ([requiredFields containsObject:SPContactFieldPhoneNumber]) {
        containsFields = containsFields && [SPPhoneNumberValidator stringIsValidPhoneNumber:self.phone forCountryCode:self.country];
    }
...
- (BOOL)containsContentForShippingAddressFields:(NSSet<SPContactField> *)desiredFields {
    return (([desiredFields containsObject:SPContactFieldName] && self.name.length > 0)
...
- (BOOL)hasValidPostalAddress {
    return (self.line1.length > 0 
            ...
            && ([SPPostalCodeValidator validationStateForPostalCode:self.postalCode
                                                         countryCode:self.country] == SPCardValidationStateValid));
}
