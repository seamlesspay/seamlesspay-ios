...
+ (NSString *)stringByReplacingSnakeCaseWithCamelCase:(NSString *)input {
  ...
  [parts enumerateObjectsUsingBlock:^(NSString *part, NSUInteger idx,
                                      __unused BOOL *stop) {
...
FOUNDATION_EXPORT NSArray *SPQueryStringPairsFromKeyAndValue(NSString *key,
                                                             id value);
...
NSString *SPQueryStringFromParameters(NSDictionary *parameters) {
  ...
  // Escape any reserved characters. due to the implementation of
  // `SPQueryStringPairsFromKeyAndValue`, it's much easier to do this now, as
  // when that function is called recursively on a dictionary, `key` will
  // (correctly) contain characters like `[]`.
...
// This function recursively converts an object into a version that is safe to
// convert into a form-encoded path string by escaping any reserved characters
// in it or its children.
id SPPercentEscapedObject(id object) {
  if (!object || object == [NSNull null]) {
    ...
    return escapedDictionary;
             ...
             (CFBooleanGetTypeID() ==
              CFGetTypeID((__bridge CFTypeRef)(object)))) {
...
NSArray *SPQueryStringPairsFromKeyAndValue(NSString *key, id value) {
  ...
  if ([value isKindOfClass:[NSDictionary class]]) {
    ...
    // is important when serializing potentially ambiguous sequences, such as an
