...
+ (UIImage *)cvcImageForCardBrand:(SPCardBrand)brand {
    NSString *imageName = brand == SPCardBrandAmex ? @"sp_card_cvc_amex" : @"sp_card_cvc";
    return [self safeImageNamed:imageName];
}

+ (UIImage *)errorImageForCardBrand:(SPCardBrand)brand {
    NSString *imageName = brand == SPCardBrandAmex ? @"sp_card_error_amex" : @"sp_card_error";
    return [self safeImageNamed:imageName];
}
...
+ (UIImage *)brandImageForFPXBankBrand:(SPFPXBankBrand)brand {
    NSString *imageName = [NSString stringWithFormat:@"sp_bank_fpx_%@", SPIdentifierFromFPXBankBrand(brand)];
...
+ (UIImage *)safeImageNamed:(NSString *)imageName
    ...
    NSURL *imageURL = [mainBundle URLForResource:imageName withExtension:@"png" subdirectory:@"Frameworks/SeamlessPayCore.framework" localization:nil];
    UIImage *image = [UIImage imageWithContentsOfFile:[imageURL path]];

    //UIImage *image = [UIImage imageNamed:imageName inBundle:nil compatibleWithTraitCollection:nil];
...
+ (UIImage *)brandImageForCardBrand:(SPCardBrand)brand 
    ...
    switch (brand) {
        ...
        case SPCardBrandDinersClub:
            imageName = shouldUseTemplate ? @"sp_card_diners_template" : @"sp_card_diners";
            break;
        case SPCardBrandDiscover:
            imageName = shouldUseTemplate ? @"sp_card_discover_template" : @"sp_card_discover";
            break;
        case SPCardBrandJCB:
            imageName = shouldUseTemplate ? @"sp_card_jcb_template" : @"sp_card_jcb";
            break;
        case SPCardBrandMasterCard:
            imageName = shouldUseTemplate ? @"sp_card_mastercard_template" : @"sp_card_mastercard";
            break;
        case SPCardBrandUnionPay:
            if ([[[NSLocale currentLocale] localeIdentifier].lowercaseString hasPrefix:@"zh"]) {
                imageName = shouldUseTemplate ? @"sp_card_unionpay_template_zh" : @"sp_card_unionpay_zh";
            } else {
                imageName = shouldUseTemplate ? @"sp_card_unionpay_template_en" : @"sp_card_unionpay_en";
            }
            break;
