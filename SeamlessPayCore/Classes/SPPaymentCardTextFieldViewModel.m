//
//  SPPaymentCardTextFieldViewModel.m
//
...
- (NSString *)compressedCardNumber {
    ...
    if ([self validationStateForField:SPCardFieldTypeNumber] == SPCardValidationStateValid) {
        // Use fragment length
        NSUInteger length = [SPCardValidator fragmentLengthForCardBrand:currentBrand];
        ...
        NSArray<NSNumber *> *cardNumberFormat = [SPCardValidator cardNumberFormatForBrand:currentBrand];
...
- (void)setExpirationMonth:(NSString *)expirationMonth {
    NSString *sanitizedExpiration = [SPCardValidator sanitizedNumericStringForString:expirationMonth];
...
- (void)setExpirationYear:(NSString *)expirationYear {
    _expirationYear = [[SPCardValidator sanitizedNumericStringForString:expirationYear] sp_safeSubstringToIndex:2];
}
...
- (SPCardValidationState)validationStateForField:(SPCardFieldType)fieldType {
    switch (fieldType) {
        ...
        case SPCardFieldTypeExpiration: {
            ...
            SPCardValidationState yearState = [SPCardValidator validationStateForExpirationYear:self.expirationYear inMonth:self.expirationMonth];
...
- (BOOL)isValid {
    return ([self validationStateForField:SPCardFieldTypeNumber] == SPCardValidationStateValid
            && [self validationStateForField:SPCardFieldTypeExpiration] == SPCardValidationStateValid
            && [self validationStateForField:SPCardFieldTypeCVC] == SPCardValidationStateValid
            && (!self.postalCodeRequired
                || [self validationStateForField:SPCardFieldTypePostalCode] == SPCardValidationStateValid));
}
