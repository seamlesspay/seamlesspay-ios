...
+ (SPCardValidationState)validationStateForUSPostalCode:(NSString *)postalCode {
    ...
    if (!firstFiveIsNumeric) {
        ...
        // ZIP+4 territory
        NSUInteger numberOfDigits = countOfCharactersFromSetInString(postalCode, [NSCharacterSet sp_asciiDigitCharacterSet]);
        ...
        if (numberOfDigits > 9) {
            ...
            if (countOfCharactersFromSetInString(separatorCharacter, [NSCharacterSet sp_asciiDigitCharacterSet]) == 0) {
...
+ (NSString *)formattedSanitizedUSZipCodeFromString:(NSString *)zipCode
    ...
    NSString *formattedString = [[SPCardValidator sanitizedNumericStringForString:zipCode] sp_safeSubstringToIndex:maxLength];
