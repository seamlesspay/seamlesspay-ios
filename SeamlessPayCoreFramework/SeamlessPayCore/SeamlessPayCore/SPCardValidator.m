...
+ (BOOL)stringIsNumeric:(NSString *)string {
    return [string rangeOfCharacterFromSet:[NSCharacterSet sp_invertedAsciiDigitCharacterSet]].location == NSNotFound;
}
...
+ (SPCardValidationState)validationStateForExpirationMonth:(NSString *)expirationMonth {
    ...
    switch (sanitizedExpiration.length) {
        ...
        case 1:
            return ([sanitizedExpiration isEqualToString:@"0"] || [sanitizedExpiration isEqualToString:@"1"]) ? SPCardValidationStateIncomplete : SPCardValidationStateValid;
        case 2:
            return (0 < sanitizedExpiration.integerValue && sanitizedExpiration.integerValue <= 12) ? SPCardValidationStateValid : SPCardValidationStateInvalid;
        default:
            return SPCardValidationStateInvalid;
    }
}

+ (SPCardValidationState)validationStateForExpirationYear:(NSString *)expirationYear inMonth:(NSString *)expirationMonth inCurrentYear:(NSInteger)currentYear currentMonth:(NSInteger)currentMonth {
        ...
        case 2: {
            if ([self validationStateForExpirationMonth:sanitizedMonth] == SPCardValidationStateInvalid) {
                ...
                if (sanitizedYear.integerValue == moddedYear) {
                    return sanitizedMonth.integerValue >= currentMonth ? SPCardValidationStateValid : SPCardValidationStateInvalid;
                } else {
                    return sanitizedYear.integerValue > moddedYear ? SPCardValidationStateValid : SPCardValidationStateInvalid;
                }
            }
        }
        default:
...
+ (SPCardValidationState)validationStateForCVC:(NSString *)cvc cardBrand:(SPCardBrand)brand {
...
+ (SPCardValidationState)validationStateForCard:(nonnull SPCardParams *)card inCurrentYear:(NSInteger)currentYear currentMonth:(NSInteger)currentMonth {
    SPCardValidationState numberValidation = [self validationStateForNumber:card.number validatingCardBrand:YES];
    NSString *expMonthString = [NSString stringWithFormat:@"%02lu", (unsigned long)card.expMonth];
    SPCardValidationState expMonthValidation = [self validationStateForExpirationMonth:expMonthString];
    NSString *expYearString = [NSString stringWithFormat:@"%02lu", (unsigned long)card.expYear%100];
    SPCardValidationState expYearValidation = [self validationStateForExpirationYear:expYearString
                                                                              inMonth:expMonthString
                                                                        inCurrentYear:currentYear
                                                                         currentMonth:currentMonth];
    SPCardBrand brand = [self brandForNumber:card.number];
    SPCardValidationState cvcValidation = [self validationStateForCVC:card.cvc cardBrand:brand];
    ...
    return incomplete ? SPCardValidationStateIncomplete : SPCardValidationStateValid;
}
