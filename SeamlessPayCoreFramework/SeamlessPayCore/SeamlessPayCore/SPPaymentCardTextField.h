...
/**
 Convenience property for creating an `SPPaymentMethodCardParams` from the currently entered information
 or programmatically setting the field's contents. For example, if you're using another library
 to scan your user's credit card with a camera, you can assemble that data into an `SPPaymentMethodCardParams`
 ...
 object is to make changes to a `SPPaymentMethodCardParams` you own (retrieved from this text field if desired),
...
/**
 ...
 @param textField The SPPaymentCardTextField that was being edited when the user pressed return
 */
